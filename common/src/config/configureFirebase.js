// Firebase configuration for both web and mobile environments
// This file automatically detects the environment and uses the appropriate Firebase SDK
...
if (isReactNative) {
    // React Native environment - use @react-native-firebase
    try {
    ...
    // Web environment - use Firebase Web SDK
    try {
        ...
        // Default config - can be overridden by passing config to FirebaseProvider
        const defaultConfig = {
...
// React Context for web environment (optional, for React components)
let FirebaseContext = null;
let FirebaseProvider = null;

if (!isReactNative) {
    // Only create React context for web environment
    const React = require('react');
    FirebaseContext = React.createContext(null);
    
    FirebaseProvider = ({ config, children }) => {
        // Re-initialize with custom config if provided
        if (config && !firebase.app) {
            try {
                const { initializeApp } = require('firebase/app');
                const { getAuth } = require('firebase/auth');
                const { getDatabase } = require('firebase/database');
