...
API_RESPONSE=$(curl -s -w "%{time_total}" -o /dev/null http://147.182.204.181:3000/api/health)
log "Latência da API: ${API_RESPONSE}s"

# Testar WebSocket
WS_RESPONSE=$(timeout 5 curl -s -w "%{time_total}" -o /dev/null http://147.182.204.181:3001 || echo "timeout")
...
# CPU e RAM da VPS
ssh root@147.182.204.181 "top -bn1 | grep 'Cpu(s)' | awk '{print \$2}' | cut -d'%' -f1" > /tmp/cpu_usage
CPU_USAGE=$(cat /tmp/cpu_usage)
log "CPU da VPS: ${CPU_USAGE}%"

# RAM da VPS
ssh root@147.182.204.181 "free | grep Mem | awk '{printf \"%.1f\", \$3/\$2 * 100.0}'" > /tmp/ram_usage
...
# Conexões Redis
REDIS_CONNECTIONS=$(ssh root@147.182.204.181 "redis-cli info clients | grep connected_clients | cut -d: -f2")
log "Conexões Redis: ${REDIS_CONNECTIONS}"

# Memória Redis
REDIS_MEMORY=$(ssh root@147.182.204.181 "redis-cli info memory | grep used_memory_human | cut -d: -f2")
...
for api in "${APIS[@]}"; do
    if [ "$api" = "nearby_drivers" ]; then
        ...
        RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" "http://147.182.204.181:3000/api/${api}?lat=-23.5505&lng=-46.6333&radius=5" || echo "000")
    else
        RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" "http://147.182.204.181:3000/api/${api}" || echo "000")
    fi
...
# Logs do PM2
ERROR_COUNT=$(ssh root@147.182.204.181 "pm2 logs leaf-api --lines 100 | grep -i error | wc -l")
...
# Uptime da VPS
UPTIME=$(ssh root@147.182.204.181 "uptime -p")
log "Uptime VPS: ${UPTIME}"

# Status do PM2
PM2_STATUS=$(ssh root@147.182.204.181 "pm2 status | grep leaf-api")
