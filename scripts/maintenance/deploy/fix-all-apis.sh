...
# Criar novo server.js completamente corrigido
cd /opt/leaf-app
...
app.post('/api/update_user_location', async (req, res) => {
    try {
        ...
        // Usar hSet com parâmetros separados
        await redisClient.hSet(`user:${userId}:location`, 'lat', locationData.lat.toString());
        await redisClient.hSet(`user:${userId}:location`, 'lng', locationData.lng.toString());
        await redisClient.hSet(`user:${userId}:location`, 'timestamp', locationData.timestamp.toString());
...
app.post('/api/update_driver_location', async (req, res) => {
    try {
        ...
        // Usar hSet com parâmetros separados
        await redisClient.hSet(`driver:${driverId}:location`, 'lat', locationData.lat.toString());
        await redisClient.hSet(`driver:${driverId}:location`, 'lng', locationData.lng.toString());
        await redisClient.hSet(`driver:${driverId}:location`, 'status', locationData.status);
        await redisClient.hSet(`driver:${driverId}:location`, 'timestamp', locationData.timestamp.toString());
