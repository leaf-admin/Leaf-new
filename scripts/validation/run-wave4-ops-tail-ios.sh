...
cleanup_active_ride_if_needed() {
  ...
  run_passenger_action cleanup_active "residual-cleanup-passenger-1"
  run_driver_action accept_offer "residual-cleanup-accept"
  sleep 3
  run_driver_action arrive_pickup "residual-cleanup-arrive"
  sleep 3
  run_driver_action start_trip "residual-cleanup-start"
  sleep 3
  run_driver_action complete_trip "residual-cleanup-complete"
  sleep 3
  run_passenger_action cleanup_active "residual-cleanup-passenger-2"
  sleep 6
}
...
ensure_driver_online() {
  log "bringing driver online via queued home automation"
  queue_driver_home_action set_online "real-trip-online"
  xcrun simctl terminate "$DRIVER_UDID" "$APP_ID" >/dev/null 2>&1 || true
  foreground_app "$DRIVER_UDID"
  if ! wait_for_driver_online "$DRIVER_UDID" "$APP_ID" 60; then
    capture_device "$DRIVER_UDID" "$ARTIFACT_DIR/driver-online-timeout.png"
    log "driver online runtime snapshot did not settle; continuing with UI grace fallback"
    sleep "$STABILIZATION_SECONDS"
  fi
  foreground_app "$DRIVER_UDID"
  sleep "$FLOW_SETTLE_SECONDS"
