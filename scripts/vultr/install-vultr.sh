...
# 🚀 SCRIPT DE INSTALAÇÃO VULTR - LEAF APP
...
cat > /usr/local/bin/backup-vultr.sh << 'EOF'
#!/bin/bash
...
chmod +x /usr/local/bin/backup-vultr.sh

# Adicionar backup ao cron
echo "0 2 * * * /usr/local/bin/backup-vultr.sh" | crontab -

echo ""
echo "✅ INSTALAÇÃO CONCLUÍDA COM SUCESSO!"
echo ""
echo "📊 Status dos serviços:"
systemctl status leaf-primary --no-pager -l
echo ""
echo "🔧 Comandos úteis:"
