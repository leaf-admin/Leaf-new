...
describe('app-link-association routes', () => {
  it('serves the marketing landing page on the public root host', async () => {
    ...
    expect(response.text).toContain('O app que respeita quem dirige. E quem paga.');
  });

  it('does not serve the marketing landing page on the API host', async () => {
    await request(createApp())
      .get('/')
      .set('Host', 'api.leaf.app.br')
      .expect(404);
  });
  ...
  it.each([
    ['/convite', 'leafapp://convite', 'Convite Leaf'],
    ['/convite/ABC123', 'leafapp://convite/ABC123', 'Convite Leaf'],
    ['/motorista/convite', 'leafapp://motorista/convite', 'Convite para dirigir na Leaf'],
    ['/motorista/convite/DRV123', 'leafapp://motorista/convite/DRV123', 'Convite para dirigir na Leaf'],
    ['/viagem', 'leafapp://viagem', 'Acompanhar viagem'],
