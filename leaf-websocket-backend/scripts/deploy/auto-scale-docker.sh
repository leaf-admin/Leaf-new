...
# Auto-Scaling para Leaf System com Docker
# Escala automaticamente baseado na carga
...
echo -e "${BLUE}🚀 AUTO-SCALING DO LEAF SYSTEM DOCKER${NC}"
echo "============================================="
...
get_load_metrics() {
    ...
    if docker ps --format "table {{.Names}}" | grep -q "leaf-websocket$"; then
        active_instances=1
    fi
...
case "${1:-monitor}" in
    "status")
        ...
        ;;
    "scale-up")
        current=$(get_load_metrics | cut -d: -f1)
        scale_up $current
        ;;
    "scale-down")
        current=$(get_load_metrics | cut -d: -f1)
