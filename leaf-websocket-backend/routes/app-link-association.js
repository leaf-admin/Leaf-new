...
const MARKETING_HOSTS = new Set(['leaf.app.br', 'www.leaf.app.br']);
...
function getFirstPathParam(req) {
  return String(req.params[0] || '').split('/').filter(Boolean)[0] || '';
}

function appendPathParam(baseHref, value) {
  const normalized = String(value || '').trim();
  return normalized ? `${baseHref}/${encodeURIComponent(normalized)}` : baseHref;
}
...
router.use('/assets', (req, res, next) => {
  ...
  const landingPageDir = findLandingPageDir();
  if (!landingPageDir) {
    return next();
  }

  return express.static(path.join(landingPageDir, 'assets'), {
    maxAge: '1h',
    fallthrough: true
  })(req, res, next);
});
...
router.get('/', (req, res, next) => {
  ...
  return res.sendFile(path.join(landingPageDir, 'index.html'));
});
...
router.get(['/convite', '/convite/*'], (req, res) => {
  const inviteCode = getFirstPathParam(req);
  res.type('html').send(renderFallbackPage({
    title: 'Convite Leaf',
    description: 'Abra o convite no app Leaf para aceitar e acompanhar seus beneficios.',
    appHref: appendPathParam('leafapp://convite', inviteCode),
    canonicalUrl: getPublicUrl(req)
  }));
});

router.get(['/motorista/convite', '/motorista/convite/*'], (req, res) => {
  const inviteCode = getFirstPathParam(req);
  res.type('html').send(renderFallbackPage({
    ...
    appHref: appendPathParam('leafapp://motorista/convite', inviteCode),
    canonicalUrl: getPublicUrl(req)
  }));
});

router.get(['/viagem', '/viagem/*'], (req, res) => {
  const tripId = getFirstPathParam(req);
  res.type('html').send(renderFallbackPage({
    title: 'Acompanhar viagem',
    description: 'Abra este link no app Leaf para acompanhar a viagem compartilhada com dados seguros.',
    appHref: appendPathParam('leafapp://viagem', tripId),
    canonicalUrl: getPublicUrl(req)
  }));
});
