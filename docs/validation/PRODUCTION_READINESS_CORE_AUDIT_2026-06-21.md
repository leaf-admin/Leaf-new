...
- **P0 finance policy divergence:** `ride-financial-contract.js` currently charges a 3% Leaf operational fee above R$ 50, while several architecture/business docs specify fixed R$ 1,49 above R$ 20. This changes driver net and cannot be resolved by an implementation guess. The runtime now fails production config validation until the active code policy id (`runtime_tiered_percent_above_50_v1`) is explicitly approved through `LEAF_APPROVED_FINANCIAL_POLICY_ID` plus `LEAF_FINANCIAL_POLICY_APPROVAL_REF`, or until the code is changed to a newly approved policy. One approved policy must still become the single source of truth, followed by quote/payment/receipt/ledger/dashboard reconciliation tests.
- **P0 extension pricing model decision:** active-trip destination extension now has a backend authority guard: `RequestRideExtensionCommand` recalculates a backend fare estimate, blocks divergent client `newFare`, persists `fareAuthority=backend_extension_estimate`, and `RespondRideExtensionCommand` refuses to generate Pix from extension requests without that authority. The remaining product decision is the exact extension-pricing model for production evidence: original pickup to new destination, current vehicle position to new destination plus executed-fare settlement, or another explicit model.
