...
| Payment blocked when no driver is available | Backend guard implemented, reservation added locally, real smoke pending | `payment-driver-availability-guard` and `create-booking-availability-precheck` exist with focused unit tests. `/api/payment/advance` checks eligible driver before creating Pix and now atomically reserves one eligible driver in Redis with a short-lived payment reservation. The payment intent and Woovi charge carry the reservation id, driver id, and expiration; the Pix expiration is bounded by the reservation TTL. The modular `createBooking` socket handler validates the same reservation before `RequestRideCommand`, and dispatch validates/consumes it before entering driver search. Drivers reserved for another payment are excluded from availability and notification dispatch. The passenger payment screen also routes `autoOpenPix` through the same availability gate before the Woovi modal can appear, preserving backend/geofence messages instead of opening Pix directly. Socket confirmation is guarded in production by provider-backed evidence. | L2 preflight must prove driver readiness before payment; no-driver path must be tested separately on device/backend to verify product copy and cleanup/refund handling. |
...
- **P0 manual driver approval scope decision closed on 2026-06-24:** manual approval cannot bypass CNH, CRLV/active vehicle, liveness, face compare/KYC, consent, or any backend activation evidence required for `canGoOnline=true`. Dashboard quick approval and `/driver-approval/approve` are audit confirmations only after canonical activation evidence is complete. Legacy mass approval/rejection remains disabled. Any future test-driver exception must be an explicit isolated test-user flow, not a production approval bypass.
- **P1 payment availability race closed locally on 2026-06-24:** `/api/payment/advance` now reserves one eligible driver in Redis before Pix, persists reservation metadata in the payment intent/charge, bounds Pix expiration by the reservation TTL, requires the reservation before paid `createBooking`, validates/consumes it before dispatch, and excludes drivers reserved for another payment from availability/notification dispatch. L2 still must prove provider webhook, real device, dashboard, and same-ride dispatch behavior.
...
- P0: manual driver approval and KYC overrides need audit reason, evidence, actor, policy trail, and complete canonical activation evidence before production use. Implemented in the current patch: `/driver-approval/approve` passes authenticated admin audit metadata, `DriverApprovalService` rejects approvals without actor/reason/provenance/evidence before calling Woovi, then fails closed unless recomputed driver activation returns `canGoOnline=true`; manual KYC unblock refuses `manualOverride` without the same audit trail.
...
| P0-006 | Prove payment availability guard with no eligible driver before payment. | Backend/payment/mobile QA | Backend guards and payment reservation implemented, pending real smoke | Payment creation is blocked before Pix by `/api/payment/advance` when no eligible driver exists. When a driver exists, `/api/payment/advance` reserves one eligible driver before creating Pix, persists reservation metadata on the payment intent/charge, and caps the charge expiration by the reservation TTL. Paid `createBooking` fails closed before `RequestRideCommand` when the payment intent lacks a valid reservation, and dispatch validates/consumes the same reservation before starting driver search. The Android smoke runner stops with `blocked_precondition:driver_unavailable` before payment when canonical pickup availability fails. The payment UI no longer lets `autoOpenPix` bypass this precheck: it calls `checkRideAvailability` first and keeps Woovi closed on no-driver/geofence failures. Focused backend/mobile unit and contract tests cover these guards. Device/backend smoke must still verify the behavior with real state. |
...
| P0-024 | Add auditable non-bypass trail for manual driver approval/KYC unblock. | Backend/dashboard/KYC | Implemented, focused backend units passed | Manual driver approval now requires actor, role, reason, provenance, evidence, previous state, next state, and recomputed canonical activation evidence with `canGoOnline=true` before Woovi/account approval. Approval fails closed when CNH, CRLV/active vehicle, liveness, face compare/KYC, consent, or activation reads are incomplete/unavailable. Dashboard KYC unblock tied to manual approval passes the same audit trail, and `kycDriverStatusService` refuses manual override unblock without audit. Focused tests cover rejection without audit, canonical evidence rejection, route propagation of admin metadata, KYC audit persistence, and payment-account creation without accidental approval. Dashboard visual audit evidence remains pending for L2/backoffice. |
