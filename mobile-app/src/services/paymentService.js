...
export const createPixCharge = async (chargeData) => {
  try {
    ...
    const response = await api.post('/api/payment/advance', {
      ...
      carType: chargeData?.carType || rideDetails?.carType || chargeData?.vehicle || null,
      vehicle: chargeData?.vehicle || chargeData?.carType || null,
      vehicleCategory: chargeData?.vehicleCategory || null,
      preferences: chargeData?.preferences || rideDetails?.preferences || {},
