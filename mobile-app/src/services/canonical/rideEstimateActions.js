...
import { calcularPedagiosPorPolyline } from './tollUtils';

const FETCH_ESTIMATE = 'FETCH_ESTIMATE';
const FETCH_ESTIMATE_SUCCESS = 'FETCH_ESTIMATE_SUCCESS';
const FETCH_ESTIMATE_FAILED = 'FETCH_ESTIMATE_FAILED';
...
function calculateBaseFare(distance, time, rateDetails) {
  ...
  const timeFare = (time / 3600) * ratePerHour;
  const subTotal = baseFare + distanceFare + timeFare + fixedFee;
  return subTotal < minFare ? minFare : subTotal;
}
...
function calculateFareEstimate(
  ...
  routePoints,
  vehicleType = 'car',
  externalTollFee = null
) {
  const baseFare = calculateBaseFare(distance, time, rateDetails);
  const tollFee = externalTollFee !== null && !isNaN(externalTollFee)
    ? externalTollFee
    : 0;
...
export const getEstimate = (bookingData) => async (dispatch) => {
  ...
  if (res) {
    ...
    settingsRef.on('value', settingdata => {
      ...
      let { totalCost, grandTotal, convenienceFee, tollFee } = calculateFareEstimate(
        distance,
        res.time_in_secs,
