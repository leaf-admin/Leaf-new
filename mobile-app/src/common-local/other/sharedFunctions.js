...
export const formatBookingObject = async (bookingData, settings) => {
  ...
  return {
    ...
    // Novo cálculo: Valor para o motorista = Tarifa total - OpFee - WooviFee
    // Pedágios não sofrem taxa operacional
    driver_share: calculateDriverShare(bookingData.estimate.estimateFare, bookingData.estimate.tollFee || 0, settings.decimal),
...
export const addActualsToBooking = async (booking, address, driverLocation) => {
  ...
  if (settings.prepaid) {
    ...
    // Novo cálculo: Valor para o motorista = Tarifa total - OpFee - WooviFee
    // Pedágios não sofrem taxa operacional
    let currentToll = booking.tollFee || 0;
    let driver_fee_val = calculateDriverShare(booking.trip_cost, currentToll, settings.decimal);

    booking.fleetCommission = fleetCommission_fee ? fleetCommission_fee : "0";
    booking.driver_share = driver_fee_val;

  }
  return booking;
};

// Nova Função para Taxas Operacionais da Leaf
export const calculateDriverShare = (tripCost, tollFee = 0, decimalPrecision = 2) => {
  ...
  return driverShare.toFixed(decimalPrecision);
}
