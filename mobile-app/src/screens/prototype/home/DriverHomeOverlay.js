...
function DriverHomeOverlay({
  ...
  const isActivationBlocked = !driverCanGoOnline;
  const sliderLabel = isActivationBlocked ? 'Ativação pendente' : driverOnline ? 'Online' : 'Ficar online';
  const goalStorageKey = useMemo(
    () => `${DRIVER_GOAL_STORAGE_PREFIX}${String(driverId || 'anonymous').trim() || 'anonymous'}`,
    [driverId]
  );
  const currentGoalProgressLabel = String(formattedDriverEarnings || '--,--').trim() || '--,--';
  const goalTargetLabel = String(Math.max(0, Math.round(Number(dailyGoal) || DEFAULT_DAILY_GOAL)));
  const sliderProgress = useRef(new Animated.Value(driverOnline && !isActivationBlocked ? 1 : 0)).current;
  ...
  const handleOpenGoalModal = () => {
    setGoalInput(String(Math.max(0, Math.round(Number(dailyGoal) || DEFAULT_DAILY_GOAL))));
  ...
  return (
    <>
      <View
        ...
        style={[styles.driverBottomCtaWrap, { bottom: insetsBottom + DRIVER_BOTTOM_CTA_OFFSET }]}
      >
        <PrototypeCard style={styles.driverBottomCard}>
          <View style={styles.driverBottomStatsRow}>
            <TouchableOpacity activeOpacity={0.86} style={styles.driverBottomGoalStatItem} onPress={handleOpenGoalModal}>
              <Text style={styles.driverBottomStatLabel}>Meta diária</Text>
              <View style={styles.driverBottomGoalValueRow}>
                <Text style={styles.driverBottomStatValuePrimary}>{currentGoalProgressLabel}</Text>
                <Text style={styles.driverBottomStatValueSecondary}> / {goalTargetLabel}</Text>
              </View>
            </TouchableOpacity>
            ...
            <View style={styles.driverBottomStatItem}>
              ...
              <Text style={styles.driverBottomTripsValue}>{Math.max(0, Number(ridesCount) || 0)}</Text>
            </View>
          </View>

          <TouchableOpacity
            activeOpacity={0.88}
            onPress={driverCanGoOnline ? onToggleOnline : onOpenActivation}
            onLayout={event => {
          ...
          >
            ...
            >
              <Ionicons
                name={driverOnline && !isActivationBlocked ? 'checkmark' : 'chevron-forward'}
                size={22}
                color={driverOnline && !isActivationBlocked ? '#0E7A35' : '#5B6470'}
              />
            </Animated.View>
          </TouchableOpacity>
        </PrototypeCard>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity activeOpacity={0.86} style={styles.modalGhostButton} onPress={() => setGoalModalVisible(false)}>
                <Text style={styles.modalGhostButtonText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity activeOpacity={0.86} style={styles.modalPrimaryButton} onPress={handleSaveGoal}>
