...
export default function CRLVUploadScreen() {
  ...
  const handleContinue = async () => {
    if (!crlvImage) {
      Alert.alert('Atenção', 'Por favor, envie uma foto do CRLV do veículo.');
      return;
    }
    ...
    try {
      ...
      Logger.log('CRLV selecionado:', crlvImage);
      Logger.log('Dados do usuário:', userData);
      
      // Simular upload
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Navegar para a próxima tela com todos os dados
      ...
      Logger.error('Erro ao processar CRLV:', error);
      Alert.alert('Erro', 'Não foi possível processar o CRLV. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <OnboardingLayout
      ...
      continueDisabled={!crlvImage || loading}
    >
      <View style={styles.container}>
        <Text style={styles.title}>Envie o CRLV do veículo</Text>
        <Text style={styles.subtitle}>
          Tire uma foto ou selecione da galeria a foto do Certificado de Registro e Licenciamento do Veículo
        </Text>
          ...
          <View style={styles.content}>
            ...
            {crlvImage && (
              <View style={styles.imagePreviewContainer}>
                <Image source={{ uri: crlvImage.uri }} style={styles.imagePreview} />
                <TouchableOpacity 
                  style={styles.removeButton}
                  onPress={() => setCrlvImage(null)}
                >
            ...
            <View style={styles.infoContainer}>
              ...
              <Text style={styles.infoText}>• Todos os dados do veículo devem estar visíveis</Text>
              <Text style={styles.infoText}>• Evite reflexos e sombras</Text>
              <Text style={styles.infoText}>• Certifique-se de que o CRLV não está vencido</Text>
              <Text style={styles.infoText}>• O veículo deve estar em seu nome ou ter autorização</Text>
            </View>
            ...
            {crlvImage && (
              <View style={styles.statusContainer}>
                ...
                <Text style={styles.statusText}>CRLV selecionado com sucesso!</Text>
              </View>
            )}
            ...
            <View style={styles.summaryContainer}>
              ...
              <View style={styles.summaryItem}>
                <MaterialCommunityIcons 
                  name={userData.cnhImage ? "check-circle" : "circle-outline"} 
                  size={20} 
                  color={userData.cnhImage ? "#4CAF50" : LEAF_GRAY} 
                />
                <Text style={[
                  styles.summaryText,
                  { color: userData.cnhImage ? "#4CAF50" : LEAF_GRAY }
                ]}>
                  CNH {userData.cnhImage ? "✓" : "Pendente"}
                </Text>
              </View>
              <View style={styles.summaryItem}>
                <MaterialCommunityIcons 
                  name={crlvImage ? "check-circle" : "circle-outline"} 
                  size={20} 
                  color={crlvImage ? "#4CAF50" : LEAF_GRAY} 
                />
                <Text style={[
                  styles.summaryText,
                  { color: crlvImage ? "#4CAF50" : LEAF_GRAY }
                ]}>
                  CRLV {crlvImage ? "✓" : "Pendente"}
                </Text>
              </View>
            </View>
          </View>
