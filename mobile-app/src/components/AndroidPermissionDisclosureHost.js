...
function getDetailItems(kind) {
  if (kind === 'background-location') {
    return [
      'Usada somente enquanto você estiver online como motorista ou durante uma corrida.',
      'Ajuda a receber chamadas, manter a navegação ativa e acompanhar a viagem com segurança.',
  ...
  if (kind === 'foreground-location') {
    return [
      'Usada para mostrar seu ponto de partida e sua posição no mapa.',
      'Ajuda a encontrar motoristas próximos, calcular rotas e acompanhar a corrida.',
      'A Leaf não usa essa permissão para anúncios.',
    ];
  }
